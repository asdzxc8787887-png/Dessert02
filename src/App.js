...
const isCustomConfig =
  MY_FIREBASE_CONFIG.apiKey && MY_FIREBASE_CONFIG.apiKey.length > 0;
...
// --- 關鍵修正：取得集合路徑 ---
// 在自有專案使用根目錄 (root collections)，在預覽環境使用巢狀目錄
...
const COLORS = {
  ...
  gold: "#D4AF37",
  goldGradient:
    "linear-gradient(135deg, #D4AF37 0%, #F5D061 50%, #B4922B 100%)",
  text: "#E0E0E0",
  subText: "#9ca3af",
  danger: "#F87171",
  success: "#34D399",
...
export default function App() {
  ...
  const filteredRecipes = useMemo(() => {
    ...
    if (productCategory !== "all") {
      list = list.filter(
        (item) => (item.category || "other") === productCategory
      );
    }
    const computedList = list.map((recipe) => {
      ...
      (recipe.ingredients || []).forEach((item) => {
        const ingredient = ingredients.find(
          (ing) => ing.id === Number(item.id)
        );
        if (ingredient) {
          const unitPrice =
            ingredient.purchase_price / ingredient.purchase_amount;
      ...
      const profitMargin =
        recipe.target_price > 0 ? (profit / recipe.target_price) * 100 : 0;
      const pctMaterial =
        (materialCost / yieldAmount / recipe.target_price) * 100;
    ...
    return (recipe.ingredients || [])
      .map((item) => {
        ...
        return {
          ...
          cost:
            (ing.purchase_price / ing.purchase_amount) * (item.amount * ratio),
        };
      })
      .filter(Boolean);
  ...
  const currentUnitCost = useMemo(() => {
    const totalBatch =
      currentEditingMaterialCost + (Number(formOtherCost) || 0);
  ...
  const handleDelete = async () => {
    if (confirm("確定要刪除嗎？")) {
      const collectionName =
        activeTab === "ingredients" ? "ingredients" : "formulas";
      try {
  ...
  const initializeSampleData = async () => {
    if (!confirm("這將會寫入範例資料，如果已有資料可能會重複，確定嗎？"))
      return;
    try {
      const batchPromises = [];
      INITIAL_INGREDIENTS.forEach((item) =>
        batchPromises.push(
          setDoc(getDocRef("ingredients", item.id.toString()), item)
        )
      );
      INITIAL_FORMULAS.forEach((item) =>
        batchPromises.push(
          setDoc(getDocRef("formulas", item.id.toString()), item)
        )
      );
  ...
  const exportToCSV = () => {
    ...
    const allComputed = sortedAllFormulas.map((recipe) => {
      ...
      const pctOther = (otherCost / yieldAmount / recipe.target_price) * 100;
      const catLabel =
        PRODUCT_CATEGORIES.find((c) => c.id === (recipe.category || "other"))
          ?.label || "其他";
      return {
        ...recipe,
        unitCost,
    ...
    const rows = allComputed.map((r) => [
      r.catLabel,
      `"${r.name}"`,
      r.yieldAmount,
      r.target_price,
      r.unitCost.toFixed(1),
      r.profit.toFixed(1),
      r.profitMargin.toFixed(1),
      `${(r.materialCost / r.yieldAmount).toFixed(1)} (${r.pctMaterial.toFixed(
        0
      )}%)`,
      `${(r.otherCost / r.yieldAmount).toFixed(1)} (${r.pctOther.toFixed(0)}%)`,
    ]);
    const csvContent =
  ...
  const openAddModal = () => {
    ...
    setFormOtherCost("0");
    setFormUnit("g");
    setFormCategory(inventoryCategory);
    setFormProductCategory(
      productCategory === "all" ? "cake" : productCategory
    );
  ...
  const openEditIngredient = (item) => {
    ...
    setFormPrice(item.purchase_price);
    setFormAmount(item.purchase_amount);
    setFormUnit(item.unit);
    setFormCategory(item.category || "ingredient");
    setModalVisible(true);
  };
  const openEditRecipe = (item) => {
    ...
    setFormId(item.id);
    setFormName(item.name);
    setFormPrice(item.target_price);
    setFormYield(item.yield_amount || 1);
    setFormOtherCost(item.other_cost || 0);
    setFormProductCategory(item.category || "other");
    setRecipeIngredients(item.ingredients || []);
    let matCost = 0;
    (item.ingredients || []).forEach((i) => {
      const ing = ingredients.find((ig) => ig.id === i.id);
      if (ing) matCost += (ing.purchase_price / ing.purchase_amount) * i.amount;
    });
    const totalCost =
      (matCost + (Number(item.other_cost) || 0)) / (item.yield_amount || 1);
    const margin =
      item.target_price > 0
        ? (((item.target_price - totalCost) / item.target_price) * 100).toFixed(
            1
          )
        : 0;
  ...
  const addIngredientToRecipe = () => {
    ...
    if (existingIndex >= 0) {
      ...
      updatedList[existingIndex].amount = parseFloat(addAmount);
      setRecipeIngredients(updatedList);
    } else {
      setRecipeIngredients([
        ...recipeIngredients,
        { id: Number(selectedIngredientId), amount: parseFloat(addAmount) },
      ]);
    }
    setAddAmount("");
  };
  const removeIngredientFromRecipe = (ingId) => {
    setRecipeIngredients(recipeIngredients.filter((item) => item.id !== ingId));
  };
  ...
  return (
    <div className="flex flex-col h-screen w-full font-sans overflow-hidden bg-[#0a0a0a] text-gray-200">
      {/* Header */}
      <header
        className="px-5 border-b border-white/10 shadow-lg z-20 flex flex-col gap-4 relative backdrop-blur-xl bg-black/40"
        style={{
      ...
      >
        <div className="flex justify-between items-center">
          <h1 className="text-xl font-bold tracking-widest flex items-center gap-2 text-white">
            {activeTab === "recipes" && (
              <div className="p-1.5 rounded-lg bg-gradient-to-br from-yellow-500/20 to-yellow-600/5">
            ...
            {activeTab === "ingredients" && (
              <div className="p-1.5 rounded-lg bg-gradient-to-br from-purple-500/20 to-purple-600/5">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="p-2.5 rounded-full bg-white/5 hover:bg-white/10 border border-white/5 transition-all active:scale-95 shadow-lg shadow-black/20"
            >
        ...
        {activeTab !== "calculator" && (
          <div className="relative group">
            ...
            <input
              type="text"
              placeholder={
                activeTab === "recipes" ? "搜尋食譜..." : "搜尋原料或包材..."
              }
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full bg-white/5 border border-white/10 rounded-xl py-3 pl-10 pr-4 text-base focus:outline-none focus:border-yellow-500/50 focus:bg-white/10 transition-all shadow-inner placeholder-gray-600"
            />
          </div>
        )}
      </header>
      ...
      <main className="flex-1 overflow-y-auto p-4 pb-32 space-y-4 z-10 scroll-smooth">
        ...
        {!loading && !user && !connectionError && (
          <div className="text-center text-gray-500 mt-20 p-4 border border-white/10 rounded-xl bg-white/5">
            ...
            <p className="text-sm">
              若您已設定 MY_FIREBASE_CONFIG，請檢查設定是否正確。
            </p>
          </div>
        )}
            ...
            <div className="flex gap-2 overflow-x-auto pb-2 -mx-4 px-4 custom-scrollbar-hide">
              {PRODUCT_CATEGORIES.map((cat) => (
                <button
                  ...
                  className={`flex-shrink-0 px-4 py-2 rounded-xl text-sm font-bold flex items-center gap-2 transition-all border ${
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
              {filteredRecipes.map((item) => (
                <div
                  ...
                  className="group relative overflow-hidden rounded-2xl p-5 border border-white/5 bg-white/5 backdrop-blur-md transition-all active:scale-[0.98] cursor-pointer hover:border-yellow-500/30 hover:shadow-2xl hover:shadow-yellow-900/10"
                >
                  <div className="absolute inset-0 bg-gradient-to-br from-white/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none" />
                  <div className="relative z-10">
                    <div className="flex justify-between items-start mb-4">
                      <div className="flex items-center gap-2">
                        <div className="p-2 bg-white/5 rounded-lg text-xl border border-white/5">
                          {
                            PRODUCT_CATEGORIES.find(
                              (c) => c.id === (item.category || "other")
                            )?.icon
                          }
                        </div>
                        <div>
                          <span className="text-[10px] text-gray-500 uppercase tracking-wide font-bold block mb-0.5">
                            {
                              PRODUCT_CATEGORIES.find(
                                (c) => c.id === (item.category || "other")
                              )?.label
                          ...
                          <h3 className="text-lg font-bold text-white line-clamp-1 group-hover:text-yellow-100 transition-colors">
                            {item.name}
                          </h3>
                        </div>
                      </div>
                    ...
                    <div className="grid grid-cols-2 gap-3 mb-4">
                      <div className="bg-black/30 p-2.5 rounded-xl border border-white/10 flex flex-col items-center justify-center">
                        ...
                        <span className="text-xl font-bold text-yellow-400 font-mono tracking-tight">
                          ${item.target_price}
                        </span>
                      </div>
                      <div
                        className={`p-2.5 rounded-xl border flex flex-col items-center justify-center relative overflow-hidden ${
                    ...
                    <div className="w-full h-2 rounded-full bg-black/40 flex overflow-hidden mb-2 shadow-inner">
                      <div
                        ...
                        style={{
                          width: `${Math.min(item.pctMaterial, 100)}%`,
                          backgroundColor: COLORS.barMaterial,
                        }}
                      />
                      <div
                        className="h-full transition-all duration-500 ease-out border-l border-black/10"
                        style={{
                          width: `${Math.min(item.pctOther, 100)}%`,
                          backgroundColor: COLORS.barOther,
                        }}
                      />
                      {item.profitMargin > 0 && (
                        <div
                          className="h-full rounded-r-full transition-all duration-500 ease-out border-l border-black/10"
                          style={{
                            width: `${Math.min(item.pctProfit, 100)}%`,
                            backgroundColor: COLORS.barProfit,
                          }}
                        />
                      )}
                    </div>
              ...
              {filteredRecipes.length === 0 && (
                <div className="text-center mt-20 col-span-full opacity-50 flex flex-col items-center gap-4">
                  <Box size={48} strokeWidth={1} />
                  <p>這裡空空的，去設定匯入範例資料吧！</p>
                </div>
              )}
            </div>
          </>
          ...
          <div className="space-y-6 max-w-lg mx-auto">
              ...
              <div className="space-y-5 relative z-10">
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    <select
                      value={calcRecipeId}
                      onChange={(e) => setCalcRecipeId(e.target.value)}
                      className="w-full bg-black/40 border border-white/10 rounded-xl py-4 px-4 text-base focus:border-blue-500/50 outline-none text-white appearance-none transition-colors"
                    >
                      <option value="">請選擇...</option>
                      {PRODUCT_CATEGORIES.filter((c) => c.id !== "all").map(
                        (cat) => {
                          const recipesInCat = sortedAllFormulas.filter(
                            (f) => (f.category || "other") === cat.id
                          );
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    <input
                      ...
                      placeholder="0"
                      className="w-full bg-black/40 border border-white/10 rounded-xl py-4 px-4 text-xl font-bold text-blue-400 placeholder-gray-700 focus:border-blue-500/50 outline-none transition-colors"
                    />
            ...
            {productionList && (
              <div className="p-5 rounded-3xl border border-white/5 bg-[#161616] shadow-inner">
                <h4 className="text-xs font-bold text-gray-500 mb-4 pb-2 border-b border-white/5 uppercase tracking-wider">
                  採購清單
                </h4>
                    ...
                    >
                      ...
                      <div className="text-right">
                        <div className="text-white font-bold text-lg font-mono">
                          {Number.isInteger(item.totalNeeded)
                            ? item.totalNeeded
                            : item.totalNeeded.toFixed(1)}{" "}
                          <span className="text-xs font-sans font-normal text-gray-500 ml-0.5">
                            {item.unit}
                          </span>
                        </div>
                        <div className="text-[10px] text-gray-600">
                          預估成本 ${item.cost.toFixed(0)}
                        </div>
                      </div>
                    </div>
                  ))}
                  <div className="mt-6 pt-4 border-t border-dashed border-white/10 flex justify-between items-center">
                    <span className="text-sm text-gray-400">總成本預估</span>
          ...
          activeTab === "ingredients" && (
            <>
              <div className="flex p-1 bg-white/5 rounded-xl border border-white/10 mb-4">
                ...
                <button
                  onClick={() => setInventoryCategory("packaging")}
                  className={`flex-1 py-2.5 rounded-lg text-sm font-bold flex items-center justify-center gap-2 transition-all ${
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredIngredients.map((item) => (
                  <div
                    ...
                    className="group rounded-2xl p-4 flex justify-between items-center cursor-pointer transition-all border border-white/5 bg-white/5 hover:bg-white/10 hover:border-purple-500/30 active:scale-[0.98]"
                  >
                    <div>
                      ...
                      <p className="text-xs font-medium px-2 py-0.5 rounded-md bg-black/30 text-gray-400 inline-block">
                        {item.purchase_amount} {item.unit} / $
                        {item.purchase_price}
                      </p>
                    </div>
                    <div className="text-right flex flex-col items-end">
                      <div className="text-lg font-bold text-purple-300 font-mono">
                        $
                        {(item.purchase_price / item.purchase_amount).toFixed(
                          2
                        )}{" "}
                ...
                {filteredIngredients.length === 0 && (
                  <div className="text-center mt-10 col-span-full opacity-50">
                    <p>這裡目前沒有資料</p>
                  </div>
                )}
              </div>
      ...
      {activeTab !== "calculator" && !connectionError && (
        <button
          ...
          className="fixed bottom-28 right-6 w-14 h-14 rounded-full shadow-[0_0_20px_rgba(212,175,55,0.3)] flex items-center justify-center z-30 transition-transform active:scale-90 group bg-gradient-to-br from-[#D4AF37] to-[#B4922B]"
        >
      ...
      {/* Bottom Nav */}
      <nav
        className="border-t border-white/10 flex fixed bottom-0 left-0 right-0 z-20 backdrop-blur-xl bg-black/60"
        style={{
          height: "auto",
          minHeight: "80px",
      ...
      >
        {[
          { id: "recipes", icon: Utensils, label: "產品利潤" },
          { id: "calculator", icon: Calculator, label: "製作計算" },
          { id: "ingredients", icon: Package, label: "庫存管理" },
        ].map((tab) => {
          ...
          return (
            ...
            >
              {isActive && (
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-16 h-12 bg-white/5 rounded-2xl blur-md" />
              )}
              <div
              ...
              >
                <tab.icon
                  size={24}
                  color={isActive ? COLORS.gold : "#6b7280"}
                  strokeWidth={isActive ? 2.5 : 2}
                  className={`drop-shadow-sm ${
                    isActive ? "drop-shadow-[0_0_8px_rgba(212,175,55,0.4)]" : ""
                  }`}
                />
              </div>
              <span
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4 bg-black/80 backdrop-blur-sm animate-fade-in">
          <div className="w-full max-w-sm rounded-3xl p-6 border border-white/10 bg-[#121212] shadow-2xl space-y-6 relative overflow-hidden">
            ...
            <div className="flex justify-between items-center mb-2 relative z-10">
              <h3 className="text-xl font-bold text-white">資料與報表</h3>
              <button
            ...
            <div className="space-y-3 relative z-10">
              <h4 className="text-xs font-bold text-gray-500 uppercase tracking-wider ml-1">
                App 應用
              </h4>
              <button
                onClick={() => {
                ...
                className="w-full py-4 rounded-2xl bg-gradient-to-r from-blue-900/40 to-blue-800/20 border border-blue-700/30 hover:border-blue-500/50 flex items-center justify-center gap-4 transition-all active:scale-[0.98] group"
              >
                <div className="p-2 rounded-full bg-blue-500/10 text-blue-400 group-hover:scale-110 transition-transform">
                  <Smartphone size={20} />
                </div>
                <div className="text-left flex-1">
                  <div className="text-white font-bold">安裝到手機 (App)</div>
            ...
            <div className="space-y-3 relative z-10">
              ...
              <button
                onClick={exportToCSV}
                className="w-full py-4 rounded-2xl bg-gradient-to-r from-emerald-900/40 to-emerald-800/20 border border-emerald-700/30 hover:border-emerald-500/50 flex items-center justify-center gap-4 transition-all active:scale-[0.98] group"
              >
                <div className="p-2 rounded-full bg-emerald-500/10 text-emerald-400 group-hover:scale-110 transition-transform">
                  <FileSpreadsheet size={20} />
                </div>
                <div className="text-left flex-1">
                  <div className="text-white font-bold">匯出報表 (CSV)</div>
            ...
            <div className="space-y-3 relative z-10">
              ...
              <button
                ...
                className="w-full py-3.5 rounded-2xl bg-white/5 border border-white/5 hover:bg-white/10 flex items-center justify-center gap-4 transition-all active:scale-[0.98]"
              >
                ...
                <div className="text-left flex-1">
                  ...
                  <div className="text-xs text-gray-500">雲端資料庫初始化</div>
                </div>
              </button>
            </div>
          </div>
          ...
          <div className="w-full max-w-sm rounded-3xl p-6 border border-blue-500/30 bg-[#121212] shadow-2xl relative">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-xl font-bold text-white flex items-center gap-2">
                <Smartphone className="text-blue-400" /> 安裝教學
              </h3>
              <button
                onClick={() => setInstallModalVisible(false)}
                ...
                <>
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1 text-gray-300 text-sm">
                      點擊瀏覽器下方的{" "}
                      <span className="text-blue-400 font-bold inline-flex items-center mx-1">
                        <Share size={14} /> 分享按鈕
                      </span>
                    </div>
                  </div>
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1 text-gray-300 text-sm">
                      往下滑動選單，找到並點選{" "}
                      <span className="text-white font-bold border border-white/20 px-1 rounded mx-1">
                        加入主畫面
                      </span>
                    </div>
                  </div>
            ...
            <button
              onClick={() => setInstallModalVisible(false)}
              className="w-full mt-8 py-3 rounded-xl bg-blue-600 hover:bg-blue-500 text-white font-bold transition-colors"
            >
              我知道了
            </button>
          </div>
      ...
      {modalVisible && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4 bg-black/80 backdrop-blur-md animate-fade-in p-4 overflow-y-auto">
          <div className="w-full max-w-md rounded-3xl border border-white/10 bg-[#18181b] shadow-2xl flex flex-col max-h-[90vh]">
            <div className="p-6 border-b border-white/5 flex justify-between items-center sticky top-0 bg-[#18181b] z-20 rounded-t-3xl">
              <h3 className="text-xl font-bold text-white flex items-center gap-2">
                {modalMode === "add" ? (
              ...
              <div className="space-y-5">
                {activeTab === "ingredients" && (
                  <div className="flex gap-2 mb-2">
                    <button
                      onClick={() => setFormCategory("ingredient")}
                    ...
                    <button
                      onClick={() => setFormCategory("packaging")}
                      className={`flex-1 py-2 rounded-lg text-xs font-bold border ${
                ...
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    className="w-full bg-black/30 border border-white/10 rounded-xl py-3 px-4 text-white focus:border-yellow-500/50 outline-none transition-colors"
                  />
                </div>
                {activeTab === "recipes" && (
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      <select
                        ...
                        className="w-full bg-black/30 border border-white/10 rounded-xl py-3 px-4 text-white outline-none focus:border-yellow-500/50 appearance-none transition-colors"
                      >
                        {PRODUCT_CATEGORIES.filter((c) => c.id !== "all").map(
                          (cat) => (
                  ...
                  <div className="flex-1 space-y-2">
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        className="w-full bg-black/30 border border-white/10 rounded-xl py-3 pl-8 pr-4 text-white font-bold outline-none focus:border-yellow-500/50"
                      />
                    </div>
                  </div>
                  {activeTab === "recipes" ? (
                    <div className="flex-1 space-y-2">
                      ...
                      <div className="relative">
                        <input
                          ...
                          placeholder="30"
                          className="w-full bg-black/30 border border-emerald-500/30 rounded-xl py-3 px-4 text-white outline-none focus:border-emerald-500"
                        />
                    ...
                    <div className="w-1/3 space-y-2">
                      ...
                      <div className="relative">
                        <select
                          ...
                          className="w-full bg-black/30 border border-white/10 rounded-xl py-3 pl-4 pr-8 text-white text-center outline-none focus:border-yellow-500/50 appearance-none transition-colors"
                        >
                ...
                {activeTab === "recipes" && (
                  <div className="flex gap-4">
                    <div className="flex-1 space-y-2">
                      <label className="text-xs font-bold text-gray-500 uppercase tracking-wider text-yellow-500/80">
                        預計產出
                      </label>
                      <div className="relative">
                        <input
                          ...
                          placeholder="1"
                          className="w-full bg-black/30 border border-yellow-500/30 rounded-xl py-3 px-4 text-white outline-none focus:border-yellow-500"
                        />
                        <span className="absolute right-4 top-1/2 -translate-y-1/2 text-xs text-gray-500">
                          份
                        </span>
                ...
                {activeTab === "recipes" && (
                  <div className="bg-white/5 p-4 rounded-xl border border-white/5 space-y-2">
                    <label className="text-xs font-bold text-pink-300 uppercase tracking-wider flex items-center gap-1">
                      <DollarSign size={12} /> 隱形成本 (總計)
                    </label>
                    <input
                      type="number"
                      value={formOtherCost}
                      ...
                      placeholder="0"
                      className="w-full bg-black/30 border border-transparent rounded-lg py-2 px-3 text-white outline-none focus:border-pink-500/50 text-sm"
                    />
                    <p className="text-[10px] text-gray-500">
                      包含水電、瓦斯、人力工時等難以單獨計算的雜支
                    </p>
                  </div>
                )}
                {activeTab === "ingredients" && (
                  <div className="flex gap-4">
                    <div className="flex-1 space-y-2">
                      ...
                      <input
                        ...
                        className="w-full bg-black/30 border border-white/10 rounded-xl py-3 px-4 text-white outline-none focus:border-yellow-500/50"
                      />
                    </div>
                ...
                <div className="pt-6 border-t border-white/10">
                  <div className="flex justify-between items-end mb-4">
                    ...
                    <div className="text-right text-xs">
                      <div className="text-gray-500">總成本預估</div>
                      <div className="text-lg font-bold text-white font-mono">
                        $
                        {(
                          currentEditingMaterialCost +
                          (Number(formOtherCost) || 0)
                        ).toFixed(0)}
                      </div>
                    </div>
                  </div>
                  <div className="flex gap-2 mb-4">
                    <select
                      ...
                      className="flex-1 bg-black/30 text-white text-sm rounded-lg px-3 outline-none border border-white/10 h-10 focus:border-white/30"
                    >
                      <option value="">選擇原料...</option>
                      <optgroup label="🥕 食材">
                        {sortedAllIngredients
                          .filter(
                            (i) => !i.category || i.category === "ingredient"
                          )
                          .map((ing) => (
                    ...
                    <input
                      ...
                      className="w-20 bg-black/30 text-white text-sm rounded-lg px-3 outline-none border border-white/10 h-10 focus:border-white/30"
                    />
                    <button
                      onClick={addIngredientToRecipe}
                      className="bg-white/10 hover:bg-white/20 text-white px-4 rounded-lg text-sm transition-colors h-10 font-bold"
                    >
                      +
                    </button>
                  </div>
                  <div className="space-y-2 max-h-40 overflow-y-auto pr-1 custom-scrollbar">
                    {recipeIngredients.map((item, idx) => {
                      ...
                      return (
                        <div
                          key={idx}
                          className="flex justify-between items-center text-sm bg-black/20 p-3 rounded-lg border border-white/5 hover:border-white/10 transition-colors"
                        >
                          ...
                          <div className="flex items-center gap-4">
                            ...
                            <button
                              onClick={() =>
                                removeIngredientFromRecipe(item.id)
                              }
            ...
            <div className="p-6 border-t border-white/5 flex gap-4 sticky bottom-0 bg-[#18181b] rounded-b-3xl">
              {modalMode === "edit" && (
                <button
                  ...
                  className="p-3.5 bg-red-500/10 text-red-400 hover:bg-red-500/20 rounded-xl transition-colors"
                >
                  <Trash2 size={20} />
                </button>
              )}
              <button
                onClick={handleSave}
                className="flex-1 py-3.5 rounded-xl font-bold text-sm text-black flex items-center justify-center gap-2 hover:opacity-90 transition-opacity shadow-lg shadow-yellow-500/20 bg-gradient-to-r from-[#D4AF37] to-[#F5D061]"
              >
                <Save size={18} /> 儲存變更
              </button>
            </div>
